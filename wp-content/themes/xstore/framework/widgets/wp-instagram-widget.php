	...
	function widget($args, $instance) {
		...
		if ($username != '') {
			...
			if ( is_wp_error($media_array) ) {
                ...
                <div class="widget null-instagram-feed <?php echo esc_attr($swiper_container); ?> slider-<?php echo $box_id ?> <?php echo esc_attr($lines); ?>" data-breakpoints="1" data-xs-slides="<?php echo esc_attr($mobile) ?>" data-sm-slides="<?php echo esc_attr($tablet_land) ?>" data-md-slides="<?php echo esc_attr($notebook) ?>" data-lt-slides="<?php echo esc_attr($large) ?>" data-slides-per-view="<?php echo esc_attr($large); ?>" data-autoplay="<?php echo esc_attr($autoplay); ?>" data-speed="300">

                    <ul class="<?php echo esc_attr($swiper_wrapper); ?> instagram-pics instagram-size-<?php echo esc_attr( $size ); ?> instagram-columns-<?php echo esc_attr( $columns ); ?> <?php if($spacing) echo 'instagram-no-space'; ?>"><?php
				foreach ( $media_array as $item ) {
					...
					if ( locate_template( 'parts/wp-instagram-widget.php' ) != '' ) {
						...
						if ($slider){
                            echo etheme_swiper_lazy_preloader();
                        }
                        echo '
                                    <img class="swiper-lazy" src="'. esc_url( $image_src ) .'"  alt="'. esc_attr( $item['description'] ) .'" title="'. esc_attr( $item['description'] ).'" width="1080" height="1080" class="'. $imgclass .'"/>
