    ...
    if ( $product_categories ) {
        ...
        if($display_type == 'menu') {
            ...
            if($display_type == 'slider') {
                ...
                $class .= 'categoriesCarousel slider-'.$box_id.' swiper-container '.$lines.' '.$no_space.'" data-breakpoints="1" data-xs-slides="'.esc_js($mobile).'" data-sm-slides="'.esc_js($tablet_land).'" data-md-slides="'.esc_js($notebook).'" data-lt-slides="'.esc_js($large).'" data-slides-per-view="'.esc_js($large).'" data-autoplay="'.esc_attr($slider_autoplay).'"';
            ...
            echo '<div class="swiper-entry">';
                ...
                if ($display_type == 'slider') {
                    ...
                    if ($pagination_type != "hide" && $display_type == 'slider') { echo '<div class="swiper-pagination hide-for-'.$hide_fo.' etheme-css" data-css=".slider-'.$box_id.' .swiper-pagination-bullet{background-color:'.$default_color.'; '. $lines.';} .slider-'.$box_id.' .swiper-pagination-bullet:hover{ background-color:'.$active_color.'; } .slider-'.$box_id.' .swiper-pagination-bullet-active{ background-color:'.$active_color.'; }"></div>'; }
                echo '</div>';
            echo '</div>';
        }
    }
