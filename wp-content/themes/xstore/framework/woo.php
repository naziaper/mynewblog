...
if(!function_exists('etheme_template_hooks')) {
	function etheme_template_hooks() {
		...
		if ( ! etheme_get_option( 'just_catalog' ) ) {
			add_action( 'woocommerce_before_add_to_cart_quantity', 'etheme_single_qutity_txt', 10 );
		}
...
// ! Add qutity text for single product
if( ! function_exists( 'etheme_single_qutity_txt' ) ) {
	function etheme_single_qutity_txt() {
		global $product;
		if ( ! $product->is_type( 'grouped' ) ) printf( '<span class="qty-span">%1s</span>', esc_html__( 'quantity', 'xstore' ) ); 
	}
}
...
if(!function_exists('etheme_wishlist_btn')) {
	function etheme_wishlist_btn() {
		...
		$out = '<div class="et-wishlist-holder type-' . $args['type'] . ' position-' . $args['position'] . '">';
...
if(!function_exists('etheme_top_cart')) {
	function etheme_top_cart($load_cart = false) {
		...
		?>
		<div class="shopping-container <?php echo esc_attr( $class ); echo ( !etheme_get_option('favicon_label_zero')) ? ' label-hidden' : ''; ?>" data-fav-badge="<?php echo (etheme_get_option('favicon_label')) ? 'enable' : 'disable'; ?>">
